 ...
 */
export function getPREventData(): PullRequestEvent {
  try {
    const eventData = fs.readFileSync(process.env.GITHUB_EVENT_PATH!, 'utf8');
    return JSON.parse(eventData) as PullRequestEvent;
  } catch (error: any) {
...
export function getModifiedFiles(): string[] {
  try {
    ...
    if (error instanceof Error) {
      ...
      const gitError = error as GitError; // Narrow down the type
 ...
 */
export function checkForDropsInMigrationsFiles(
  migrationFiles: string[],
  detectTableOrColumnDrop: (content: string) => boolean
): boolean {
